  ...
  return (
    <div style={{ padding: '28px 36px' }}>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid #ffffff0D', marginBottom: 22 }}>
        {['overview','quote','signing','contract','handoff','audit'].map(t => (
          <div key={t} className={`tab ${tab === t ? 'on' : ''}`} onClick={() => setTab(t)}>
            {t === 'overview' ? 'Overview' : t === 'quote' ? '✦ Quote Builder' : t === 'signing' ? 'Signatures' : t === 'contract' ? 'Contract' : t === 'handoff' ? 'Team Handoff' : 'Audit Trail'}
          </div>
        ))}
      </div>
