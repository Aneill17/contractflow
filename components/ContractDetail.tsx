    ...
    <div style={{ padding: '28px 36px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          ...
          {/* Stage 1 = awaiting client approval — show reminder instead of advance */}
          {c.stage === 1 && (
            <button className="btn btn-ghost btn-sm" onClick={async () => {
              await fetch('/api/remind', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contract_id: c.id }) })
              showToast('Reminder sent to client')
            }}>✉ Send Reminder</button>
          )}
          {c.stage === 1 && (
            <div style={{ padding: '6px 14px', borderRadius: 7, background: '#C9A84C11', border: '1px solid #C9A84C33', fontFamily: 'IBM Plex Mono', fontSize: 11, color: '#C9A84C88' }}>
              Awaiting Client Approval
            </div>
          )}
          {/* Stage 2 = quote approved — show Generate Contract */}
          {c.stage === 2 && !(c as any).generated_contract && (
            <button className="btn btn-gold btn-sm" onClick={async () => {
              showToast('Generating contract with AI...')
              const res = await fetch('/api/generate-contract', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contract_id: c.id }) })
              const data = await res.json()
              if (data.success) { showToast('Contract generated — review in Contract tab'); await onRefresh() }
              else showToast('Generation failed', 'error')
            }}>✦ Generate Contract</button>
          )}
          {/* Stage 3+ normal advance */}
          {c.stage >= 3 && c.stage < 8 && <button className="btn btn-gold btn-sm" onClick={advance}>Advance Stage →</button>}
          {/* Stage 0 advance is allowed (move from Request to Quote Builder) */}
          {c.stage === 0 && <button className="btn btn-gold btn-sm" onClick={advance}>Advance Stage →</button>}
        </div>
      </div>
      ...
      {/* QUOTE BUILDER */}
      {tab === 'quote' && (
        <QuoteBuilder contract={c} onUpdate={onUpdate} onRefresh={onRefresh} showToast={showToast} />
      )}
